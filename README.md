...
Or, if you already committed, use the **1-Click Undo (Soft Reset)** to instantly un-commit your changes without losing any work.
...
### 5. Seamless UX Refinements
- **Global Git Identity**: Set your global `user.name` and `user.email` directly from the Settings Modal without opening a terminal.
- **Power User Shortcuts**: Press `Ctrl+S` to Stage All, and `Ctrl+Enter` inside the commit box to instantly commit.
- **Beautiful Empty States**: TyeGit celebrates your clean working tree with gorgeous visual feedback instead of empty boxes.
